...
export default function InstructorCourseDetail() {
  ...
  const handleSaveSyllabus = () => {
    if (courseId) {
      apiClient.put(`/courses/${courseId}/syllabus`, { modules })
        .then(() => alert('Đã lưu đề cương thành công!'))
        .catch(err => alert('Lỗi khi lưu đề cương: ' + err.message))
    }
  }
      ...
      <div className="flex items-center gap-4 mb-6 border-b border-slate-100 pb-4">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-[#1F3864]">Chỉnh sửa Đề cương Khóa học {course?.title ? ` - ${course.title}` : ''}</h1>
            <span className="px-2.5 py-1 text-[10px] font-bold uppercase rounded-full bg-slate-100 text-slate-500">Draft</span>
          </div>
          <p className="text-sm text-slate-500 mt-1">Sắp xếp chương học, tải lên video và tài liệu</p>
        </div>
        <button 
          onClick={handleSaveSyllabus}
          className="ml-auto px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-sm"
        >
          Lưu Đề Cương
        </button>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div key={mod.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-4 animate-[fadeIn_0.3s_ease]">
              <div className="bg-slate-50 p-4 border-b border-slate-200 flex items-center justify-between group">
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openModuleModal(mod)} className="p-1.5 text-slate-500 hover:text-blue-600 rounded-lg"><Edit size={16}/></button>
                  <button onClick={() => deleteModule(mod.id)} className="p-1.5 text-slate-500 hover:text-red-600 rounded-lg"><Trash2 size={16}/></button>
                </div>
              </div>
                  ...
                  <div key={les.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-slate-300 hover:shadow-sm transition-all group/item cursor-pointer">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{les.type} {les.duration && `• ${les.duration}`}</p>
                      </div>
                    </div>
                    <button className="text-slate-600 hover:text-slate-600"><MoreVertical size={16}/></button>
                  </div>
                ))}
          ...
          {modules.length === 0 && (
            <div className="text-center p-12 bg-white border border-slate-200 border-dashed rounded-2xl">
              ...
              <button onClick={() => openModuleModal()} className="px-4 py-2 bg-[#1F3864] text-slate-900 rounded-lg text-sm font-semibold inline-flex items-center gap-2"><Plus size={16}/> Tạo chương đầu tiên</button>
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-2xl w-[400px] shadow-2xl p-6">
            ...
            <form onSubmit={saveModule}>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 text-sm font-semibold text-slate-900 bg-[#1F3864] rounded-lg">Lưu lại</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl w-[450px] shadow-2xl p-6">
            ...
            <form onSubmit={saveLesson} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                ...
                <button type="submit" className="px-4 py-2 text-sm font-semibold text-slate-900 bg-[#1F3864] rounded-lg">Thêm bài giảng</button>
              </div>
            </form>
          </div>
        </div>
